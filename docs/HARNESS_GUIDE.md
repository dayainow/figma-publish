...
| 증상 | 원인 | 대응 |
| --- | --- | --- |
| Page 1에 레이어 0개 | 캡처 미완료 또는 `use_figma` 조립 전 한도 차단 | 아래 재캡처 절차 |
| 캡처 완료 메시지인데 빈 화면 | 브라우저에서 capture script 미로드 (hash URL만 `open`한 경우) | `layout.tsx`에 capture script 포함 후 dev server 재시작 |
| Hero 이미지만 blank | `use_figma`만 실행, imageHash 미이전 | 병렬 캡처 후 imageHash 복사 |

**재캡처 절차 (데모 앱)**

```bash
cd examples/demo-app && npm run dev   # http://localhost:3456
```

1. Cursor에서 `generate_figma_design({ fileKey: "KBndkTK7y4oN4xpDqk1Fro" })` → `captureId` 수신
2. 반환된 hash URL로 `localhost:3456` 열기 (또는 채팅에 URL·fileKey 넣고 “다시 캡처해줘”)
3. 5초 간격으로 `captureId` 폴링 → `completed`
4. 이어서 `use_figma`로 `Demo / Home` 프레임 조립 → 캡처 레이어 삭제

데모 앱 `layout.tsx`는 dev 모드에서 capture script를 자동 로드합니다.
